    ...
    pathname.startsWith(`/${locale}/gallery`)
      ...
      : pathname.startsWith(`/${locale}/about`)
        ...
        : pathname.startsWith(`/${locale}/services`)
          ? t("services")
          : pathname.startsWith(`/${locale}/booking`)
            ? t("booking")
            : pathname.startsWith(`/${locale}/model`)
              ? t("model")
              : locale === "hu"
                ? "Budapest"
                : "Budapest";
    ...
    >
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8">
        <Link href={`/${locale}`} className="group flex flex-col leading-none">
          <span className="mb-1 text-[10px] font-semibold uppercase tracking-[0.24em] text-neutral-500 transition-colors group-hover:text-neutral-950">
            {currentSection}
          </span>
          <span className="font-serif text-2xl tracking-tight text-neutral-950 transition-transform duration-300 group-hover:-translate-y-0.5 sm:text-3xl">
            Richard Foto
          </span>
        </Link>

        <div className="hidden items-center gap-7 text-xs font-semibold uppercase tracking-[0.16em] text-neutral-700 md:flex">
          <Link
            href={`/${locale}/gallery`}
          ...
          <Link
            href={`/${locale}/model`}
            className="flex max-w-24 flex-col items-center justify-center border border-neutral-950/35 px-3 py-1.5 text-center text-[9px] font-semibold uppercase leading-tight tracking-[0.12em] text-neutral-700 transition-all duration-300 hover:-translate-y-0.5 hover:border-neutral-950 hover:text-neutral-950 active:scale-95"
          >
            <span>{locale === "hu" ? "Jelentkezz" : "Apply"}</span>
            <span>{locale === "hu" ? "modellnek" : "as model"}</span>
          </Link>
        </div>
      ...
      {open && (
        <div className="flex flex-col gap-7 border-t border-neutral-950/10 bg-[#fbfaf7] px-6 py-10 text-lg font-medium shadow-[0_24px_80px_rgba(10,10,10,0.12)] md:hidden">
          ...
          <Link
            href={`/${locale}/model`}
            onClick={() => setOpen(false)}
            className="mt-3 flex flex-col items-center justify-center bg-neutral-950 py-5 text-center text-sm uppercase leading-tight tracking-[0.2em] text-white"
          >
            <span>{locale === "hu" ? "Jelentkezz" : "Apply"}</span>
            <span>{locale === "hu" ? "modellnek" : "as model"}</span>
          </Link>
